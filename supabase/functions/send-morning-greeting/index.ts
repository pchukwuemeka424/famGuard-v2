// Supabase Edge Function: Send Morning Greeting to All Users
// Deploy with: supabase functions deploy send-morning-greeting
// This function sends a warm morning greeting push notification to all registered users
// Scheduled to run daily at 8:00 AM via pg_cron
...
serve(async (req) => {
  ...
  try {
    ...
    // Use data field to check for morning_greeting_system type
    const today = new Date().toISOString().split('T')[0] // YYYY-MM-DD format
    const { data: todayGreetings, error: checkError } = await supabaseClient
      .from('notifications')
      .select('id')
      .eq('data->>type', 'morning_greeting_system')
    ...
    if (checkError) {
      ...
      console.log('⏭️ Morning greetings already sent today. Skipping to prevent duplicates.')
      return new Response(
        JSON.stringify({ 
          success: true,
          sent: 0,
          failed: 0,
          total: 0,
          message: 'Morning greetings already sent today',
          skipped: true
        }),
        { 
          status: 200, 
    ...
    // Use a database function to atomically check and create a lock
    // This prevents race conditions in concurrent executions
    const { data: lockResult, error: lockError } = await supabaseClient
      .rpc('check_and_lock_morning_greeting', { 
        p_today: today 
      })
      .single()

    // If the function doesn't exist or fails, use simple check
    if (lockError) {
      if (lockError.message?.includes('function') && lockError.message?.includes('does not exist')) {
        console.warn('⚠️ Lock function not available, using simple check')
      } else {
        console.warn('⚠️ Lock function error:', lockError.message)
      }
      
      // Simple check - if any morning_greeting_system exists today, skip
      ...
      if (simpleCheck && simpleCheck.length > 0) {
        console.log('⏭️ Morning greetings already sent today. Skipping.')
        return new Response(
          JSON.stringify({ 
            success: true,
            sent: 0,
    ...
    } else if (lockResult === false || lockResult?.locked === false) {
      // Lock indicates greetings already sent
      console.log('⏭️ Morning greetings already sent today (lock check). Skipping.')
      return new Response(
        JSON.stringify({ 
          success: true,
          sent: 0,
    ...
    // Prepare morning greeting message
    const title = '🌅 Good Morning!'
    const body = 'FamGuard wishes you a safe and wonderful day ahead. Stay protected!'
    ...
    const messages = uniqueTokens.map(token => ({
      ...
      priority: 'default',
      channelId: 'default', // Use default channel for morning greetings
    }))
    ...
    console.log(`📤 Sending ${messages.length} morning greeting notifications...`)
    ...
    console.log(`✅ Successfully sent ${successfulSends} morning greetings, ${failedSends} failed`)
    ...
    // Update the lock record to mark completion
    if (successfulSends > 0) {
      try {
        const { error: updateError } = await supabaseClient
          .from('notifications')
          .update({
            title: '🌅 Morning Greeting Sent',
            body: `Morning greetings sent to ${successfulSends} users`,
            data: {
              type: 'morning_greeting_system',
          ...
          .eq('data->>type', 'morning_greeting_lock')
          .gte('created_at', `${today}T00:00:00.000Z`)
          .lt('created_at', `${today}T23:59:59.999Z`)

        if (updateError) {
          console.warn('⚠️ Could not update notification record:', updateError.message)
        }
      } catch (error) {
        console.warn('⚠️ Error updating notification record:', error)
      }
    }
    ...
    return new Response(
      JSON.stringify({ 
        ...
        message: `Morning greeting sent to ${successfulSends} users, ${notificationsInserted} notifications inserted`,
        timestamp: new Date().toISOString()
      }),
      { 
        status: 200, 
    ...
    console.error('❌ Error in send-morning-greeting:', error)
    return new Response(
      JSON.stringify({ 
